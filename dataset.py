...
class Multi30kDataset(Dataset):
    ...
    def build_vocab(self, min_freq=2):
        ...
        train_dataset = load_dataset("bentrevett/multi30k")["train"]
        ...
        src_idx = len(self.special_tokens)
        tgt_idx = len(self.special_tokens)
    ...
    def numericalize_src(self, tokens):
        return [
            self.src_vocab.get(token, self.src_unk_idx)
            for token in tokens
        ]

    def numericalize_tgt(self, tokens):
        return [
            self.tgt_vocab.get(token, self.tgt_unk_idx)
            for token in tokens
        ]
